  ...
  return (
    <div className="mb-6 space-y-2 rounded-xl border border-mp-border bg-mp-surface p-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        />
        <Button type="button" size="sm" variant="outline" onClick={handleCopy} disabled={!url}>
          {copied ? "Copié !" : "Copier"}
        </Button>
        <Button type="button" size="sm" variant="ghost" onClick={handleRegenerate} disabled={isPending}>
          Régénérer
        </Button>
      </div>
    </div>
